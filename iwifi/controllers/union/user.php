...
class User extends CI_Controller {
	...
	public function index()
	{
        ...
        $navConfig = $this->config->item('nav');
		$navType = $navConfig['navType'];
		// Get adertisement for this page.
		$data['ad'] = $this->aiwifi->get_advertisement($data['lang_type'], 4);

		// Get top navs here.
		$data['nav_top'] = $this->aiwifi->comm_list_where('navigator', array('default' => $navType['top']), 100, 0, 'nav_sort', 'ASC');
		...
		ob_clean();
		$this->load->view('/union/user/nav_new',$data);
	}
...
#获取图片的相关信息
	...
	public function passwd(){
		...
		$data['source'] = $data['base'].'source/';
		$uid = $this->session->userdata('uid');
		$this->load->view('union/user/user_passwd',$data);
