...
class Store extends CI_Controller {
	...
	public function map($id)
	{
		$id = (int)$id;
		$data = $_REQUEST;
		$store_id = $id?$id:(int)$data['store_id'];
		$stores = storeX::get();
		...
		//提交地图信息
			$mapInfo = array(
					...
					'noShowStoreList' => trim($data['noShowStoreList']),
				  );
			//处理上传的文件
			if(!empty($stores[$store_id]['mapInfo'])){
				$mapInfo['pic'] = trim($stores[$store_id]['mapInfo']['pic']);
			}
			$upload = new upload('pic');
			if($upload->isUpload()){
				$path = "./data/store/$store_id";
				$filename = $upload->save($path);
				$mapInfo['pic'] = "/$path/$filename";
			}
			$mapInfo = json_encode($mapInfo);
			storeX::update($store_id,'mapInfo',$mapInfo);
			js::msgskip('操作成功','');
		}
		...
		$this->load->view('control_panel/store/map', $data);
	}
	...
	public function index(){
    	...
    	$config['base_url'] = $data['base'].'control_panel/store/index/';
    	...
    	$data['stores'] = $this->aiwifi->comm_list_where('store', array(), $config['per_page'],$this->uri->segment(4), 'store_id', 'DESC');
    	
    	$this->load->view('control_panel/store/store_list', $data);
		
	}
	
	// Add new store page.
	public function new_store_show(){
    	...
    	$this->load->view('control_panel/store/store_new', $data);
