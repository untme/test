...
$(document).ready(function(){
    ...
    $(".backtouserlist").click(function(){
    	...
    	window.location.href="<?= $base; ?>control_panel/activity/index/";
	...
	<div class="content-box"><!-- Start Content Box -->
		...
		<div class="content-box-content">
			<div class="tab-content default-tab" id="tablefield">
				...
				<form id="store_basic" enctype="multipart/form-data" method="post" target="_self" action="<?= $base; ?>control_panel/activity/processEditForm/<?= $activity['pk_activity'];?>">
                
                <input type="hidden" name="action" value="action" />
                
                <p>
					<span>活动名称 : </span>
					<input class="text-input small-input" value="<?= $activity['name'] ;?>" type="text" id="activity_name" name="name" />
                    <br /><small>活动名称</small>
				</p>
                <p>
					<span>活动图片 : </span>
					<input type="file" value="" class="text-input small-input" name="focus[]" id="focus_0">
                    <input type="hidden" value="<?= urlencode(json_encode($activity['focus']));?>" name="old_focus" >
					<br /><small>商家的活动图片</small>
					<?php
					if(!empty($activity['focus'])){
					?>
						<img src="<?= $activity['focus'][0] ;?>" width="120px" />
					<?php 
					}
					?>
				</p>	
				...
				<p>
					<span>手工排序 : </span>
					<input class="text-input small-input" value="<?= $activity['porder'] ;?>" type="text" id="activity_porder" name="porder" />
                    <br /><small>序号越小，越靠前显示.</small>
				</p>				
			
				<p>
					<span>商家简介 : </span>
					<textarea class="text-input medium-input" id="content" name="description" style="height:300px; width:800px;"><?= $activity['description'] ;?></textarea>
				</p>  
				...
				<p>
					...
					<select id="type" name="state">
			        	<option <?php if($activity['state']==1){ ?> selected="" <?php };?> value="1">活动正常</option>
			        	<option <?php if($activity['state']==0){ ?> selected="" <?php };?>  value="0">活动关闭</option>
			        	<option <?php if($activity['state']==2){ ?> selected="" <?php };?>  value="2">置顶排序</option>
			        	<option <?php if($activity['state']==3){ ?> selected="" <?php };?>  value="3">活动页置顶</option>
			        	<option <?php if($activity['state']==4){ ?> selected="" <?php };?>  value="4">品牌页置顶</option>
			        </select>	
				</p>
                <p>
