	...
	<div class="content-box"><!-- Start Content Box -->
		...
		<div class="content-box-content">
			<div class="tab-content default-tab" id="tablefield">
				...
				<form id="store_basic" enctype="multipart/form-data" method="post" target="_self" action="<?= $base; ?>control_panel/app/processEditForm/<?= $app['pk_app'];?>">
                
                <input type="hidden" name="action" value="action" />
                
                <p>
					<span>APP名称 : </span>
					<input class="text-input small-input" value="<?= $app['name'];?>" type="text" id="app_name" name="name" />
                    <br /><small>APP的名称</small>
				</p>
                <p>
					<span>App缩略图 : </span>
					<input type="file" class="text-input small-input" name="focus" id="file">
                    <input type="hidden"  value="<?= $app['focus'];?>" name="old_focus" id="old_focus"/>
					<br /><small>App缩略图</small>
					<?php if(!empty($app['focus'])){ ?><img src="<?= $app['focus'];?>" width="100px"/><?php }?>
				</p>	
				<p>
					<span>App分类 : </span>
					<select id="type" name="type">		
						<?php foreach($appType as $k=>$v){?>
						<option <?php if($app['type']==$k){?>selected="selected"<?php }?> value="<?= $k;?>"><?= $v;?></option>
						<?php }?>
			        </select>		        
				...
				<p>
					<span>手工排序 : </span>
					<input class="text-input small-input" value="<?= $app['porder'];?>" type="text" id="app_porder" name="porder" />
                    <br /><small>序号越小，越靠前显示.</small>
				</p>
				<p>
					<span>下载地址 : </span>
					<input class="text-input small-input" value="<?= $app['url'];?>" type="text" id="app_url" name="url" />
                    <br /><small>App的下载路径.</small>
				</p>

				<p>
					<span>APP简介 : </span>
					<textarea class="text-input medium-input" id="description" name="description" style="height:380px; width:800px;"><?= $app['description'];?></textarea>
				</p>  

				<div class="shopTitle">状态设置</div>
				<p>
					<span>展示状态 : </span>
					<input type="checkbox" <?php if($app['state']==$appState['on']){?>checked="checked"<?php }?> value="<?= $appState['on'];?>" name="state"> 开启展示状态
				</p>
                <p>
