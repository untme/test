	...
	<div class="content-box"><!-- Start Content Box -->
		...
		<div class="content-box-content">
			<div class="tab-content default-tab" id="tablefield">
				...
				<form id="store_basic" enctype="multipart/form-data" method="post" target="_self" action="<?= $base; ?>control_panel/brand/processEditForm/<?= $brand['pk_brand'];?>">
                
                <input type="hidden" name="action" value="action" />
                
                <p>
					<span>品牌名称 : </span>
					<input class="text-input small-input" value="<?= $brand['name'];?>" type="text" id="brand_name" name="name" />
                    <br /><small>品牌名称</small>
				</p>
                <p>
					<span>品牌LOGO : </span>
					<input type="file" value="" class="text-input small-input" name="logo" id="brand_logo">
                    <input type="hidden" value="<?= $brand['logo'];?>" class="text-input small-input" name="old_logo" id="old_brand_logo">
					<br /><small>商家的品牌LOGO</small>
					<?php if(!empty($brand['logo'])){?><img src="<?= $brand['logo'];?>" width="400px"/><?php }?>
				</p>	
                <p>
					<span>活动焦点 : </span>
					<input type="file" value="" class="text-input small-input" name="focus" id="brand_focus">
                    			<input type="hidden" value="<?= $brand['focus'];?>" class="text-input small-input" name="old_focus" id="old_brand_focus">
                    <br /><small>商家的活动焦点</small>
					<?php if(!empty($brand['focus'])){?><img src="<?= $brand['focus'];?>"/><?php }?>
				</p>	
                <p>
					<span>活动横幅 : </span>
					<input type="file" value="" class="text-input small-input" name="hrad" id="brand_hrad">
                    			<input type="hidden" value="<?= $brand['hrad'];?>" class="text-input small-input" name="old_hrad" id="old_brand_hrad">
                    <br /><small>商家的活动横幅</small>
					<?php if(!empty($brand['hrad'])){?><img src="<?= $brand['hrad'];?>"/><?php }?>
				</p>
					...
					<select id="brand_aiwifi_floor_aiwifi_mall_pk_mall" name="aiwifi_floor_aiwifi_mall_pk_mall">
						...
						<optgroup label="<?= $k; ?>"></optgroup>
							<?php foreach($malls as $mall): ?>
								<option <?php if($mall['pk_mall']==$brand['aiwifi_floor_aiwifi_mall_pk_mall']){ ?>selected="selected"<?php } ?> value="<?= $mall['pk_mall'];?>"><?= $mall['name'];?></option>
							<?php endforeach; ?>
						<?php endforeach; ?>
			        </select>		        
				...
				<p>
					...
					<select id="brand_aiwifi_floor_pk_floor" name="aiwifi_floor_pk_floor">
						<?php foreach($floorList as $k=>$v): ?>
						<option <?php if($v['pk_floor']==$brand['aiwifi_floor_pk_floor']){ ?>selected="selected"<?php } ?> value="<?= $v['pk_floor']; ?>"><?= $v['name']; ?></option>	
						<?php endforeach; ?>
			        </select>		        
				...
				<p>
					<span>所属类别 : </span>
						...
						<option <?php if($k==$brand['type']){ ?>selected="selected"<?php } ?> value="<?= $k; ?>"><?= $v; ?></option>	
						<?php endforeach; ?>
						</select>		        
                    <br /><small>标明所属类别</small>
				</p>
				<p>
					<span>手工排序 : </span>
					<input class="text-input small-input" value="<?= $brand['porder'];?>" type="text" id="brand_poroder" name="porder" />
                    <br /><small>序号越小，越靠前显示.</small>
				</p>				
				<p>
					<span class="display:block">品牌简介 : </span>
					<textarea class="text-input medium-input" id="brand_description" name="description" style="height:200px; width:800px;"><?= $brand['description'];?></textarea>
				</p>  
				<p>
					<span>品牌详情 : </span>
					<textarea class="text-input medium-input" id="brand_detail" name="detail" style="height:300px; width:800px;"><?= $brand['detail'];?></textarea>
				</p>  
				<p>
					<span>活动详情 : (在活动规定时间内会显示，非活动时间段隐藏) </span>
					<textarea class="text-input medium-input" id="action_detail" name="action_detail" style="height:300px; width:800px;"><?= $brand['action_detail'];?></textarea>
				</p>
				...
				<p>
					<span>展示状态 : </span>
					<input type="checkbox" <?php if($brand['state']==$brandState['on']){?>checked="checked"<?php }?> value="<?= $brandState['on'];?>" name="state"> 开启展示状态
				</p>
                <p>
