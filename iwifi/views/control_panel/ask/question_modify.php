	...
	<div class="content-box"><!-- Start Content Box -->
		...
		<div class="content-box-content">
			<div class="tab-content default-tab" id="tablefield">
				<form id="checkfrom" method="post" target="_self" action="<?= $base; ?>control_panel/ask/modify_question/<?= $lang['lang_type_id']; ?>/<?= $qinfo['aq_id']; ?>">
                ...
                <p>
					<span>问题名称 : </span>
					<input class="text-input small-input" type="text" id="aq_title" 
                    name="aq_title" value="<?= $qinfo['aq_title']; ?>" /><br />
                    <small>问题的名称.不同语言使用对应的语言填写.</small>
				</p>
                
				<p>
					<span>问题类型 : </span>
					<select name="aq_type">
                    <option value="1" <?php if ($qinfo['aq_type'] == 1){echo 'selected';}?>>单选</option>
                    <option value="2" <?php if ($qinfo['aq_type'] == 2){echo 'selected';}?>>多选</option>
					</select>
				</p>
				
                <p>
					<span>问题级别 : </span>
                    <input name="aq_level" type="text" value="<?php echo $qinfo['aq_level'];?>" size="10" maxlength="10"/>
                    ( 如果填写"0"表示基础问题; 如果是扩展问题,则填写所属基础问题的指定答案ID )
                </p>
                <p>
					<span>顺序排号 : </span>
                    <input name="sequence" type="text" value="<?php echo $qinfo['sequence'];?>" size="10" maxlength="10"/>
					( 按照排列序号从小到大的顺序来显示 )
                </p>
                
                <p>
					<input class="button submitNewQuestion" type="button" value="修 改" />
